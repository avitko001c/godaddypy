...
from godaddypy.utils import six

import godaddypy.exceptions
...
def multi_file_load_config(*filenames):
    ...
    for filename in filenames:
        ...
        except godaddypy.exceptions.ConfigNotFound:
            continue
...
def raw_config_parse(config_filename, parse_subsections=True):
    ...
    if path is not None:
        ...
        if not os.path.isfile(path):
            raise godaddypy.exceptions.ConfigNotFound(path=_unicode_path(path))
        cp = six.moves.configparser.RawConfigParser()
        try:
            cp.read([path])
        except (six.moves.configparser.Error, UnicodeDecodeError):
            raise godaddypy.exceptions.ConfigParseError(
