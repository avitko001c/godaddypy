...
from godaddypy.exceptions import requests
from godaddypy.exceptions.requests.packages import urllib3
...
def _exception_from_packed_args(exception_cls, args=None, kwargs=None):
    ...
    # Ideally, this would also be a class method on the GoDaddyPyError
...
class GoDaddyPyError(Exception):
    """
    The base exception class for GoDaddyPy exceptions.
...
class DataNotFoundError(GoDaddyPyError):
    """
...
class ApiVersionNotFoundError(GoDaddyPyError):
    """
...
class HTTPClientError(GoDaddyPyError):
...
class ConnectionError(GoDaddyPyError):
    fmt = 'An HTTP Client failed to establish a connection: {error}'


class InvalidIMDSEndpointError(GoDaddyPyError):
...
class NoCredentialsError(GoDaddyPyError):
    """
...
class PartialCredentialsError(GoDaddyPyError):
    """
...
class CredentialRetrievalError(GoDaddyPyError):
    """
...
class UnknownSignatureVersionError(GoDaddyPyError):
    """
...
class ServiceNotInRegionError(GoDaddyPyError):
    """
...
class BaseEndpointResolverError(GoDaddyPyError):
...
class ProfileNotFound(GoDaddyPyError):
    """
...
class ConfigParseError(GoDaddyPyError):
    """
...
class ConfigNotFound(GoDaddyPyError):
    """
...
class MissingParametersError(GoDaddyPyError):
    """
...
class ValidationError(GoDaddyPyError):
    """
...
class ParamValidationError(GoDaddyPyError):
...
class UnknownServiceStyle(GoDaddyPyError):
    """
...
class PaginationError(GoDaddyPyError):
    fmt = 'Error during pagination: {message}'


class OperationNotPageableError(GoDaddyPyError):
    fmt = 'Operation cannot be paginated: {operation_name}'


class ChecksumError(GoDaddyPyError):
...
class UnseekableStreamError(GoDaddyPyError):
...
class WaiterError(GoDaddyPyError):
...
class InvalidDNSNameError(GoDaddyPyError):
...
class InvalidS3AddressingStyleError(GoDaddyPyError):
...
class UnsupportedS3ArnError(GoDaddyPyError):
...
class UnsupportedS3ControlArnError(GoDaddyPyError):
...
class InvalidHostLabelError(GoDaddyPyError):
...
class UnsupportedOutpostResourceError(GoDaddyPyError):
...
class UnsupportedS3AccesspointConfigurationError(GoDaddyPyError):
...
class InvalidEndpointDiscoveryConfigurationError(GoDaddyPyError):
...
class UnsupportedS3ControlConfigurationError(GoDaddyPyError):
...
class InvalidRetryConfigurationError(GoDaddyPyError):
...
class InvalidS3UsEast1RegionalEndpointConfigError(GoDaddyPyError):
...
class InvalidSTSRegionalEndpointsConfigError(GoDaddyPyError):
...
class StubResponseError(GoDaddyPyError):
...
class InvalidConfigError(GoDaddyPyError):
    fmt = '{error_msg}'
...
class RefreshWithMFAUnsupportedError(GoDaddyPyError):
    fmt = 'Cannot refresh credentials: MFA token required.'


class MD5UnavailableError(GoDaddyPyError):
    fmt = "This system does not support MD5 generation."


class MetadataRetrievalError(GoDaddyPyError):
...
class CapacityNotAvailableError(GoDaddyPyError):
    fmt = (
        'Insufficient request capacity available.'
    )


class InvalidProxiesConfigError(GoDaddyPyError):
    fmt = (
